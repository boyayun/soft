 ...
 * @details By default Output mode is set to "Asserted while counter is active"
 * By default Input Clock source selected is "Count Rising Edge of Primary 
 * Source"
 * This use case will configure the QTimer in Counter Mode with following 
 * characteristics:
 * - Counts up
 * - Rollover enabled
 ...
 * @details This use case will configure the QTimer in Input Capture Mode with 
 * following characteristics. Default Primary Clock source rising edge is selected. 
 * Output mode is also fixed at "asserted while counter is active". 
 ...
 * @details This use case will configure the QTimer in Output Compare Mode with 
...
typedef enum 
{
  ...
  CAPTURE_CALLBACK =3   ///< Input capture event detected 
 ...
 * @return  TRUE      input edge flag was set (*ptr is written by value from
 *                    of capture register)\n
...
#define TMRCH0_GetINPUT       ((TMR0_SCTRL & TMR_SCTRL_INPUT_MASK)?TRUE:FALSE)
#define TMRCH1_GetINPUT       ((TMR1_SCTRL & TMR_SCTRL_INPUT_MASK)?TRUE:FALSE)
#define TMRCH2_GetINPUT       ((TMR2_SCTRL & TMR_SCTRL_INPUT_MASK)?TRUE:FALSE)
#define TMRCH3_GetINPUT       ((TMR3_SCTRL & TMR_SCTRL_INPUT_MASK)?TRUE:FALSE)
/***************************************************************************//*!
 ...
 * @param   ch        CH0,CH1,CH2,CH3
 * @return  TRUE      external input pin asserted (logic 1)\n
